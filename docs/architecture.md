...
アプリケーションは `main.py` (エントリポイント), `constants.py` (定数), `main_window.py` (MainWindowクラス), `widgets/` (UIウィジェット群) に分割されている。
...
現在このビルドにゲート中の機能はない（`FEATURE_FLAG_DEFINITIONS` は空の辞書）。仕組み自体は今後の実験的機能のために残してある。
...
`tests/test_chromium_timing.py` が AST 解析と動作テストで
このパターンの違反を自動検出する。
