...
- [ ] **パッケージングが成功した**
  ...
  - 実行ファイル(.exe)が存在
- [ ] **健全性チェックを実行した**
  - `npm run verify` を実行（または `npm run package:verify`）
  - すべてのチェックに合格
  - エラーがあれば修正してから再実行
...
## 🚀 新機能: Linux環境でのWindows exeテスト

### 自動健全性チェック（最速・推奨）

```bash
# ビルド & パッケージング & チェックを一気に実行
...
詳細: `docs/WINDOWS_TESTING_ON_LINUX.md`

### Wine で実際にexeを起動（完全なテスト）
...
プッシュ時に自動でWindows環境でテストが実行されます。
- GitHub Actionsタブで結果を確認
- ビルド成果物をダウンロード可能
...
# 11. GitHub Actions で Windows環境での自動テスト
# （GitHubのActionsタブで確認）
```
