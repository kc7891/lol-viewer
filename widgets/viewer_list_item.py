...
class PendingPickListItemWidget(QWidget):
    """Custom widget for pending enemy picks that don't have a viewer yet"""

    CLOSE_GLYPH = "\u00d7"  # ×

    def __init__(self, champion_name: str, parent_window: 'MainWindow'):
        super().__init__()
    ...
    def init_ui(self):
        ...
        # Champion name
        champ_name = self.champion_name.strip().title() or "(Empty)"
        self.name_label = QLabel(champ_name)
        self.name_label.setStyleSheet(f"""
            QLabel {{
                color: #e2e8f0;
        ...
        # Page type label
        self.type_label = QLabel("COUNTER")
        self.type_label.setStyleSheet(f"""
            QLabel {{
                color: #6d7a8a;
                font-size: {sz['font_sidebar_type']};
        ...
        # Open viewer button (visible on hover only)
        self.open_button = QPushButton("+")
        self.open_button.setToolTip("Open viewer")
        self.open_button.setFixedSize(sz["icon_size_sidebar_btn"], sz["icon_size_sidebar_btn"])
        ...
        self.open_button.clicked.connect(
            lambda: self.parent_window.materialize_enemy_viewer(self.champion_name)
        )
        ...
        self.close_button.clicked.connect(
            lambda: self.parent_window.remove_pending_pick(self.champion_name)
        )
        self.close_button.setVisible(False)
        layout.addWidget(self.close_button)
    ...
    def enterEvent(self, event):
        """Show open and close buttons on hover"""
        self.open_button.setVisible(True)
        self.close_button.setVisible(True)
        super().enterEvent(event)

    def leaveEvent(self, event):
        """Hide open and close buttons when not hovering"""
        self.open_button.setVisible(False)
        self.close_button.setVisible(False)
        super().leaveEvent(event)
    ...
    def _load_champion_icon(self):
        """Load champion icon from image cache"""
        champion_name = self.champion_name.strip()
        if not champion_name:
            return
        champ = self.parent_window.champion_data.get_champion(champion_name)
        if not champ:
